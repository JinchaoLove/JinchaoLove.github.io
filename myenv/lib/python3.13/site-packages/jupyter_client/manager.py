...
class KernelManager(ConnectionFileMixin):
    ...
    async def _async_finish_shutdown(
        ...
        try:
            self.log.info("start 1 wait for kernel died")
            await asyncio.wait_for(
                self._async_wait(pollinterval=pollinterval), timeout=waittime / 2
            )
            self.log.info("end 1 wait for kernel died")
        except asyncio.TimeoutError:
            self.log.info("timeout error, 1 wait for kernel died")
            ...
            self.log.info("send kernel sigterm")
        try:
            await asyncio.wait_for(
                self._async_wait(pollinterval=pollinterval), timeout=waittime / 2
            )
    ...
    async def _async_restart_kernel(self, now: bool = False, newports: bool = True, **kw) -> None:
        ...
        t_all = Cat.new_transaction("kernel", "restart_test")
        t = Cat.new_transaction("kernel", "shutdown_kernel_test")
        shutdown_f = asyncio.ensure_future(ensure_async(self.shutdown_kernel(now=now, restart=False)))
        t.complete()
        if newports:
        ...
        t = Cat.new_transaction("kernel", "start_kernel_test")
